...
class F5RussianTTSEngine(BaseTTSEngine):
    def __init__(self, manager: F5RussianManager | None = None) -> None:
        self.manager = manager or F5RussianManager()
        self._process: subprocess.Popen[str] | None = None
        self._stdout_thread: threading.Thread | None = None
        self._stderr_thread: threading.Thread | None = None
        self._messages: queue.Queue[dict[str, Any]] = queue.Queue()
        self._stderr_lines: list[str] = []
        self._worker_config: tuple[str, str, str, str, str, str] | None = None
    ...
    def validate(self, voice_config: dict[str, Any]) -> None:
        if not self.manager.is_installed():
            raise TTSEngineError(
                "F5-TTS Russian is selected, but its runtime dependencies or model "
        ...
        if not reference_audio.is_file():
            raise TTSEngineError(
                "F5-TTS Russian requires a valid reference audio file."
            )
        if not str(voice_config.get("reference_text", "") or "").strip():
            raise TTSEngineError(
                "F5-TTS Russian requires the exact transcript of the reference "
                "audio. This prevents an additional ASR model from being downloaded."
            )
        device = str(voice_config.get("device", "auto"))
        if device not in {"auto", "cuda", "cpu", "mps"}:
            raise TTSEngineError(f"Unsupported F5-TTS Russian device: {device}")
    ...
    def synthesize_to_wav(
        ...
        request: dict[str, Any] = {
            ...
            "ref_audio": str(resolve_large_asset_path(str(
                voice_config.get("reference_audio_path", "") or ""
            ))),
            "ref_text": str(voice_config.get("reference_text", "") or "").strip(),
            "nfe_step": int(voice_config.get("nfe_step", 32) or 32),
            "speed": max(
                0.25,
                min(
                    4.0,
                    float(voice_config.get("engine_speed", 1.0) or 1.0)
                    * float(voice_config.get("speed", 1.0) or 1.0),
                ),
            ),
        ...
        if not output_wav.is_file() or output_wav.stat().st_size == 0:
            raise TTSEngineError(
                "F5-TTS Russian completed but did not create a valid WAV file."
            )
        return output_wav
    ...
    def _ensure_worker(
        ...
        model_id = str(voice_config.get("model", "f5tts_v1_base_v2"))
        model = self.manager.model(model_id)
        config = (
            model.repo_id,
    ...
    def _start_worker(
        ...
        model_repo, checkpoint_file, revision, device, cache_dir, deps_dir = config
        command = [
        ...
        self.log_callback("Starting F5-TTS Russian persistent worker.")
        try:
            ...
            raise TTSEngineError(f"Could not start F5-TTS Russian: {exc}") from exc
        ...
        with self._lock:
            ...
            self._stdout_thread = threading.Thread(
                ...
                args=(process,),
                name="F5RussianStdout",
                daemon=True,
            )
            self._stderr_thread = threading.Thread(
                target=self._read_stderr,
                args=(process,),
                name="F5RussianStderr",
                daemon=True,
            )
    ...
    def _wait_for_ready(self, process: subprocess.Popen[str]) -> None:
        while True:
            ...
            if message_type == "ready":
                ...
                stress = str(message.get("stress", "")).strip()
                suffix = ", ".join(part for part in (device, stress) if part)
                if suffix:
                    self.log_callback(f"F5-TTS Russian worker ready ({suffix}).")
                else:
                    self.log_callback("F5-TTS Russian worker ready.")
                return
        ...
        while True:
            ...
            try:
                ...
                if process.poll() is not None:
                    raise TTSEngineError(
                        "F5-TTS Russian worker exited unexpectedly. "
            ...
            if message.get("type") == "stdout_closed":
                if process.poll() is not None:
                    raise TTSEngineError(
                        "F5-TTS Russian worker stdout closed. "
                        + self._worker_error_details(process)
                    )
                continue
            if message.get("type") == "raw":
                self.log_callback(f"F5-TTS Russian - {message.get('message', '')}")
                continue
            return message
    ...
    def _send_request(
        ...
        if process.stdin is None:
            raise TTSEngineError("F5-TTS Russian worker stdin is not available.")
        try:
            process.stdin.write(json.dumps(request, ensure_ascii=False) + "\n")
            process.stdin.flush()
        except OSError as exc:
            raise TTSEngineError(
                "Could not send request to F5-TTS Russian worker. "
                + self._worker_error_details(process)
            ) from exc
    ...
    def _log_worker_message(self, message: dict[str, Any]) -> None:
        ...
        if message_type == "timing":
            ...
            self.log_callback(f"F5-TTS Russian timing - {label}: {elapsed:.3f} s")
    ...
    def _raise_for_worker_message(self, message: dict[str, Any]) -> None:
        ...
        if message_type == "shutdown":
            raise TTSEngineError("F5-TTS Russian worker shut down unexpectedly.")
